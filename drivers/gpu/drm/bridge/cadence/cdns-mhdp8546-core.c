...
{
	u32 dptx_car, source_pkt_car, source_aif_car;
	int ret;
	...
	if (enable) {
		dev_dbg(mhdp->dev, "%s: Enable clocks for AIF, VIF and PIF\n", __func__);
		...
		dev_dbg(mhdp->dev, "%s: Disable clocks for AIF, VIF and PIF\n", __func__);
				    ...
				    struct drm_bridge_state *bridge_state)
{
	...
	/* Enable clocks for audio, video and packet interfaces */
				     ...
				     struct drm_bridge_state *bridge_state)
{
	...
	/* Disable clocks for audio, video and packet interfaces */
...
static void cdns_mhdp_audio_stop(struct cdns_mhdp_device *mhdp)
{
...
static int cdns_mhdp_audio_mute(struct cdns_mhdp_device *mhdp, bool enable)
{
	...
	if (enable) {
		dev_dbg(mhdp->dev, "Muting audio\n");
		ret = cdns_mhdp_reg_write_bit(mhdp, CDNS_DP_VB_ID(stream_id), 4,
					      1, CDNS_DP_VB_ID_AUDIO_MUTE);
	} else {
		dev_dbg(mhdp->dev, "Unmuting audio\n");
		ret = cdns_mhdp_reg_write_bit(mhdp, CDNS_DP_VB_ID(stream_id), 4, 1, 0);
	}
	if (ret)
