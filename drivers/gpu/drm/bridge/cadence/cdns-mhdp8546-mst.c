...
static void cdns_mhdp_set_rate_governing(struct cdns_mhdp_bridge *mhdp_bridge,
					 const bool enable)
{
	struct cdns_mhdp_device *mhdp = mhdp_bridge->mhdp;
	const u8 stream_id = mhdp_bridge->stream_id;
...
cdns_mhdp_get_payload(struct cdns_mhdp_bridge *mhdp_bridge)
{
...
static int cdns_mhdp_set_act_enable(struct cdns_mhdp_device *mhdp)
{
...
#if 0	// Wrong way of reg read. Need mailbox access:
	...
	do {
		ret = cdns_mhdp_reg_read(mhdp, CDNS_DP_MTPH_ACT_STATUS, &mtph_status);
		if (ret < 0) {
			dev_err(mhdp->dev, "Failed to read CDNS_DP_MTPH_ACT_STATUS: %d\n", ret);
			return ret;
		}
...
static int
cdns_mhdp_mst_connector_late_register(struct drm_connector *connector)
{
...
static void
cdns_mhdp_mst_connector_early_unregister(struct drm_connector *connector)
{
	struct cdns_mhdp_connector *mhdp_connector = to_mhdp_connector(connector);
...
static const struct drm_connector_funcs cdns_mhdp_mst_connector_funcs = {
	...
	//TODO Not sure if below will be required. To check
...
static int cdns_mhdp_mst_get_modes(struct drm_connector *connector)
{
	...
	/* Note: TODO **********
	   drm_dp_mst_get_edid() calls drm_get_edid to get EDID. This may fail for
           our case as we get edid through FW. For SST also, we are using custom edid
	   read function through 
	   drm_do_get_edid(connector, cdns_mhdp_get_edid_block, mhdp);
	   This might need to be used here also. Check during testing.
	 */
...
{
	// TODO Check if this is needed at all, as there is only one encoder for a connector.
	// connector->possible_encoders is set in function call drm_connector_attach_encoder()
	struct cdns_mhdp_connector *mhdp_connector;
...
/*
	// TODO mode_valid can implement calling drm_dp_calc_pbn_mode() here.
			  ...
			  const struct drm_display_mode *mode)
{
	...
	mhdp_connector = mhdp_bridge->connector;	// TODO check how to get connector here....api like get_new_connector ??
	if (mhdp_bridge->stream_id > -1) {
		DRM_ERROR("stream id is attached before bridge is enabled\n");
		return;
	}

	stream_id = bridge->encoder->crtc->index;	// TODO How to get crtc->index here??

	mhdp_bridge->pbn = drm_dp_calc_pbn_mode(mode->clock, bpp, mhdp->is_dsc);	// To be done in atomic_check

	slots = drm_dp_find_vcpi_slots(&mhdp->mst_mgr, mhdp_bridge->pbn);	// To be done in atomic_check

	ret = drm_dp_mst_allocate_vcpi(&mhdp->mst_mgr,			// can be in atomic_enable??
				       mhdp_connector->port,
	...
	// TODO Set line threshold for stream. Done in core driver. See function DP_MST_AllocateVcpi()
...
void cdns_mhdp_mst_probe(struct cdns_mhdp_device *mhdp,
...
cdns_mhdp_set_act_enable(struct cdns_mhdp_device *mhdp)
{
	...
	ret = readl_poll_timeout(mhdp->regs + CDNS_DP_MTPH_STATUS, reg,		// Swap: Wrong way of reg read. Need mailbox access
				 ((reg & CDNS_DP_MTPH_ACT_STATUS) == 0), 0,
				 30);
	if (ret) {
		dev_err(mhdp->dev,
...
void cdns_mhdp_mst_enable(struct drm_bridge *bridge)
{
	...
	mhdp_bridge->pbn = drm_dp_calc_pbn_mode(mode->clock, bpp);	// in atomic_check

	slots = drm_dp_find_vcpi_slots(&mhdp->mst_mgr, mhdp_bridge->pbn);	// in atomic_check
	ret = drm_dp_mst_allocate_vcpi(&mhdp->mst_mgr,			// can be in atomic_enable??
				       mhdp_connector->port,
				       mhdp_bridge->pbn, slots);
	if (ret == false) {
				...
				const char *pathprop)
{
	...
	mhdp_connector->bridge =
		cdns_mhdp_create_fake_mst_bridge(mhdp, mhdp_connector);
...
static const struct drm_dp_mst_topology_cbs mst_cbs = {
	...
	.register_connector = cdns_mhdp_mst_cbs_register_connector,	// Not required
	.destroy_connector = cdns_mhdp_mst_cbs_destroy_connector,	// Not required
};

static void cdns_mhdp_set_mst_enable(struct cdns_mhdp_device *mhdp, bool enable)	// DONE
{
	u32 reg_val;
...
int cdns_mhdp_mst_init(struct cdns_mhdp_device *mhdp)	// DONE
{
...
void cdns_mhdp_mst_deinit(struct cdns_mhdp_device *mhdp)	// DONE
{
	if (mhdp->is_mst) {
		mhdp->is_mst = false;
		drm_dp_mst_topology_mgr_set_mst(&mhdp->mst_mgr,
