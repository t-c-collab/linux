...
#define SET_HW_STATE(mhdp, newstate)					\
	({								\
		dev_dbg(mhdp->dev, "%s:%d hw_state %d => %d\n",		\
			__func__, __LINE__, mhdp->hw_state, newstate);	\
		WARN_ON(!spin_is_locked(&mhdp->start_lock));		\
		mhdp->hw_state = newstate;				\
	})
			    ...
			    struct cdns_mhdp_device *mhdp)
{
	...
	SET_HW_STATE(mhdp, MHDP_HW_LOADING);
	spin_unlock(&mhdp->start_lock);
	...
	SET_HW_STATE(mhdp, MHDP_HW_READY);
...
wait_loading:
	spin_lock(&mhdp->start_lock);
	if (mhdp->hw_state == MHDP_HW_LOADING) {
		spin_unlock(&mhdp->start_lock);
		mdelay(100);
		goto wait_loading;
	...
	SET_HW_STATE(mhdp, MHDP_HW_STOPPED);
	spin_unlock(&mhdp->start_lock);
