				  ...
				  u8 opcode, u16 size, u8 *message)
{
	...
	for (i = 0; i < 4; i++) {
		ret = cdns_dp_mailbox_write(mhdp, header[i]);
		if (ret)
			return ret;
	}

	for (i = 0; i < size; i++) {
		ret = cdns_dp_mailbox_write(mhdp, message[i]);
		if (ret)
			return ret;
	}
...
int cdns_mhdp_set_firmware_active(struct cdns_mhdp_device *mhdp, bool enable)
{
	...
	for (i = 0; i < sizeof(msg); i++) {
		ret = cdns_dp_mailbox_write(mhdp, msg[i]);
		if (ret)
...
/**
 * cdns_mhdp_link_probe() - probe a DisplayPort link for capabilities
 * @aux: DisplayPort AUX channel
 * @link: pointer to structure in which to return link capabilities
 *
 * The structure filled in by this function can usually be passed directly
 * into cdns_mhdp_link_power_up() and cdns_mhdp_link_configure() to power up and
 * configure the link based on the link's capabilities.
 *
 * Returns 0 on success or a negative error code on failure.
 */
static
int cdns_mhdp_link_probe(struct drm_dp_aux *aux, struct cdns_mhdp_link *link)
{
	u8 values[3];
	int err;
...
static int cdns_mhdp_link_up(struct cdns_mhdp_device *mhdp)
{
	...
	cdns_mhdp_link_probe(&mhdp->aux, &mhdp->link);
