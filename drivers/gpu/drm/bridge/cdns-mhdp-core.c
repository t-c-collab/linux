...
static irqreturn_t mhdp_irq_handler(int irq, void *data)
{
	...
	bool bridge_attached;
	int ret;
	u32 framer, available_bandwidth;
	...
	if (bridge_attached && (sw_ev0 & CDNS_DPTX_HPD)) {
		...
		if (ret > 0) {
			...
			if(mhdp->bridge_enabled) {
				available_bandwidth = mhdp->link.num_lanes * (mhdp->link.rate / 100);
				if (mhdp->required_bandwidth  > available_bandwidth) {
			...
			dev_dbg(mhdp->dev, "Failed to get HPD status %d\n", ret);
		}
			    ...
			    int lanes, int rate)
{
	...
	mhdp->required_bandwidth = req_bw;
	max_bw = lanes * rate;
