...
const struct dispc_features dispc_k2g_feats = {
	/* XXX: k2g has 4375000 Hz min pclk */
	...
	/*
	 * XXX According TRM the RGB input buffer up to 2560 should work
	 *     on 3 taps, but in practice they do not appear to work.
	 */
	.scaling = {
	...
	.vp_feat = { .color = {
			.has_ctm = false, /* XXX ctm customizaion for K2G */
			.gamma_size = 256,
...
static const u16 tidss_am6_common_regs[DISPC_COMMON_REG_TABLE_LEN] = {
...
const struct dispc_features dispc_am6_feats = {
	.max_pclk_kHz = {
		[DISPC_VP_DPI] = 165000,
	...
	.common = "common",
	.common_regs = tidss_am6_common_regs,
...
static void dispc_am6_ovr_set_plane(struct dispc_device *dispc,
				    u32 hw_plane, u32 hw_videoport,
				    u32 x, u32 y, u32 zpos)
{
				...
				u32 x, u32 y, u32 zpos)
{
	...
	case DISPC_K2G:
		...
		break;
	case DISPC_AM6:
		dispc_am6_ovr_set_plane(dispc, hw_plane, hw_videoport,
					 x, y, zpos);
		break;
	case DISPC_J721E:
				    ...
				    const struct dispc_csc_coef *csc)
{
	...
	if (regval[7] != 0)
		dev_warn(dispc->dev, "%s: No post offset suport for %s\n",
			 __func__, csc->name);
...
static void dispc_am6_vp_write_gamma_table(struct dispc_device *dispc,
					   u32 hw_videoport)
{
	u32 *table = dispc->vp_data[hw_videoport].gamma_table;
	u32 hwlen = dispc->feat->vp_feat.color.gamma_size;
				       ...
				       u32 hw_videoport)
{
	...
	case DISPC_K2G:
		...
		break;
	case DISPC_AM6:
		dispc_am6_vp_write_gamma_table(dispc, hw_videoport);
		break;
	case DISPC_J721E:
...
int dispc_runtime_resume(struct dispc_device *dispc)
{
	...
	if (dispc->feat->subrev == DISPC_AM6)
