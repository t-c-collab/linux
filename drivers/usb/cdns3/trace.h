...
DECLARE_EVENT_CLASS(cdns3_log_request,
	...
	TP_printk("%s: req: %p, req buff %p, length: %u/%u %s%s%s, status: %d,"
		  " trb: [start:%d, end:%d: virt addr %pa], flags:%x SID: %d",
...
DECLARE_EVENT_CLASS(cdns3_stream_split_transfer_len,
	...
	TP_printk("%s: req: %p,request length: %u actual length: %u  SID: %d",
		__get_str(name), __entry->req, __entry->length, __entry->actual,
		__entry->stream_id
	)
);
...
DECLARE_EVENT_CLASS(cdns3_log_trb,
	...
	TP_STRUCT__entry(
		...
		__field(u32, control)
		__field(u32, type)
		__field(unsigned, last_stream_id)
	),
	TP_fast_assign(
