...
/**
 * devm_of_phy_optional_get() - lookup and obtain a reference to an optional phy.
 * @dev: device that requests this phy
 * @np: node containing the phy
 * @con_id: name of the phy from device's point of view
 *
 * Gets the phy using devm_of_phy_get(), and associates a device with
 * it using devres. On driver detach, release function is invoked on
 * the devres data, then, devres data is freed. This differs to
 * devm_of_phy_get() in that if the phy does not exist, it is not
 * considered an error and -ENODEV will not be returned. Instead the
 * NULL phy is returned, which can be passed to all other phy consumer
 * calls.
 */
