...
int cdns_torrent_phy_configure_multilink(struct cdns_torrent_phy *cdns_phy)
{
	...
	for (node = 0; node < cdns_phy->nsubnodes; node++) {
		...
		if (phy_t1 == TYPE_DP) {
			...
			ret = cdns_torrent_dp_multilink_init(cdns_phy, &cdns_phy->phys[node]);
			if (ret)
				return ret;
		}
