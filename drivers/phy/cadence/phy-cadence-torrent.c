					     ...
					     u32 rate, bool ssc)
{
	...
	case 2700:
	case 5400:
		cdns_dp_phy_write_phy(cdns_phy,
		...
		if (ssc)
			cdns_torrent_dp_enable_ssc_19_2mhz(cdns_phy,
							   0x033A,
							   0x006A);
		break;
 ...
 */
static
void cdns_torrent_dp_enable_ssc_25mhz(struct cdns_torrent_phy *cdns_phy,
				      u32 ctrl2_val)
{
	cdns_dp_phy_write_phy(cdns_phy, CMN_PLL0_SS_CTRL1_M0, 0x0001);
	cdns_dp_phy_write_phy(cdns_phy, CMN_PLL0_SS_CTRL1_M0, ctrl2_val);
...
static int cdns_torrent_dp_run(struct cdns_torrent_phy *cdns_phy)
{
	...
	ret = cdns_torrent_dp_set_power_state(cdns_phy,
					      cdns_phy->num_lanes,
					      POWERSTATE_A2);
	if (ret)
		return ret;
				      ...
				      bool enable)
{
	...
	cdns_dp_phy_write_dp(cdns_phy,
			     PHY_PMA_XCVR_PLLCLK_EN, pll_reg_write_val);

	/* Wait for acknowledgment from PHY. */
	ret = cdns_phy_read_dp_poll_timeout(cdns_phy,
					    PHY_PMA_XCVR_PLLCLK_EN_ACK,
					    read_val,
					    (read_val & pll_reg_bits) ==
					    pll_reg_write_val,
					    0, POLL_TIMEOUT_US);
	ndelay(100);
	return ret;
...
static void cdns_torrent_dp_set_a0_pll(struct cdns_torrent_phy *cdns_phy,
				       u32 num_lanes)
{
	u32 pwr_state = cdns_dp_phy_read_dp(cdns_phy,
					    PHY_PMA_XCVR_POWER_STATE_REQ);
	u32 pll_clk_en = cdns_dp_phy_read_dp(cdns_phy,
					     PHY_PMA_XCVR_PLLCLK_EN);

	/* Lane 0 is always enabled. */
	pwr_state &=
	~(PMA_XCVR_POWER_STATE_REQ_LN_MASK << PHY_POWER_STATE_LN_0);
	pll_clk_en &= ~0x01U;

	if (num_lanes > 1) {
		/* lane 1 */
		pwr_state &=
		~(PMA_XCVR_POWER_STATE_REQ_LN_MASK << PHY_POWER_STATE_LN_1);
		pll_clk_en &= ~(0x01U << 1);
	}
	...
	if (num_lanes > 2) {
		/* lanes 2 and 3 */
		pwr_state &=
		~(PMA_XCVR_POWER_STATE_REQ_LN_MASK << PHY_POWER_STATE_LN_2);
		pwr_state &=
		~(PMA_XCVR_POWER_STATE_REQ_LN_MASK << PHY_POWER_STATE_LN_3);
		pll_clk_en &= ~(0x01U << 2);
		pll_clk_en &= ~(0x01U << 3);
	}
				     ...
				     struct phy_configure_opts_dp *dp)
{
	...
	value |=
	((~lane_mask) << PMA_TX_ELEC_IDLE_SHIFT) & PMA_TX_ELEC_IDLE_MASK;
				    ...
				    struct phy_configure_opts_dp *dp)
{
	...
	if (ret)
		return ret;
	ret = cdns_torrent_dp_set_power_state(cdns_phy,
					      dp->lanes, POWERSTATE_A2);
	if (ret)
		return ret;
	ret = cdns_torrent_dp_set_power_state(cdns_phy,
					      dp->lanes, POWERSTATE_A0);
	if (ret)
		return ret;
	ndelay(900);
